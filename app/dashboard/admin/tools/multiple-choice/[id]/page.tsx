...
import type { MultipleChoiceConfig } from "@/types/tool"
...
export default function MultipleChoiceViewPage({ params }: ToolViewPageProps) {
  ...
  const Icon = getToolTypeMeta("multiple_answer")?.icon || FileText
  const toolTypeLabel = getToolTypeLabel(
    getToolTypeByKey("multiple_answer"),
    lang
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {lang === "ar" ? "المهام" : "Assignments"}
          </CardTitle>
          <CardDescription>
            {lang === "ar"
              ? "عرض جميع المهام في صفحة "
